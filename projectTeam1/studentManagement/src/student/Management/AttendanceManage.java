	...
	public void studentArrive() {// 학생 도착 메소드
		...
		while (flag) {
			...
			for (int i = 0; i < data.getStudentList().size(); i++) {
				sNum = data.getStudentList().get(i).getPhoneNum();// .get(i).getPhoneNum(); // studentList - i인덱스의 전화번호를
																	// 받음
			...
			if (count == 0) {
				...
				if (answer.equals("n")) {
					...
					studentArrive();	// y면 자기메소드 다시 시작(다시입력부터), 왜냐하면 입력자체를 반복문안에서가 아닌 메소드 시작할떄에 했습니다
				}
